        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((product) => (
              <article
                key={product.id}
                className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition duration-300 group flex flex-col"
              >
                {/* Product Image */}
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  {/* Buttons Fix: Added items-stretch and w-full to ensure equal height and alignment */}
                  <div className="mt-auto pt-6 flex items-stretch gap-3">
                    <Link href={`/product/${product.slug}`} className="flex-1">
                      <button className="w-full h-full bg-black text-white py-3 rounded-xl font-semibold hover:bg-gray-800 transition text-sm">
                        View Product
                      </button>
                    </Link>

                    <Link href="/compare" className="flex-none">
                      <button className="h-full px-5 border border-gray-300 rounded-xl hover:bg-gray-100 transition text-sm font-semibold">
                        Compare
                      </button>
                    </Link>
                  </div>
